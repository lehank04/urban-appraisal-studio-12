  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <StringSelectWithCustom label="Tipo de inmueble" value={i.tipoInmueble} onChange={(v) => set('tipoInmueble', v)} options={TIPOS_INMUEBLE} />
        <StringSelectWithCustom label="Régimen de propiedad" value={i.regimen} onChange={(v) => set('regimen', v)} options={REGIMENES} />

        <StringSelectWithCustom label="Propósito del avalúo" value={i.proposito} onChange={(v) => set('proposito', v)} options={PROPOSITOS} />
        <TextField label="Propietario" value={i.propietario} onChange={(v) => set('propietario', v)} />
        <StringSelectWithCustom label="Moneda" value={i.moneda} onChange={(v) => set('moneda', v)} options={['US$', 'C$']} />
