...
const CATS: { key: FotoCategoria; label: string }[] = [
  ...
  { key: 'lindero_norte', label: 'Lindero Norte' },
  { key: 'lindero_sur',   label: 'Lindero Sur' },
  { key: 'lindero_este',  label: 'Lindero Este' },
  { key: 'lindero_oeste', label: 'Lindero Oeste' },
  { key: 'macrolocalizacion', label: 'Macrolocalización' },
  { key: 'microlocalizacion', label: 'Microlocalización' },
  { key: 'comparables_terreno', label: 'Comparables · Terreno' },
  { key: 'comparables_inmueble',label: 'Comparables · Construido' },
  { key: 'planos',  label: 'Planos / croquis' },
...
export function StepFotos({ avaluo }: { avaluo: Avaluo }) {
  ...
  const move = (cat: FotoCategoria, id: string, dir: -1 | 1) => {
    setFotos(cat, (fs) => {
      ...
      const copy = [...fs]; [copy[i], copy[j]] = [copy[j], copy[i]]; return copy;
    });
  };
  ...
  return (
    <div className="space-y-5">
      <header>
        <div className="text-xs uppercase tracking-widest text-primary">Capítulo VII · INMOVAL</div>
        <h2 className="text-xl font-semibold">Anexo fotográfico</h2>
        <p className="text-sm text-muted-foreground">Sube imágenes en las categorías oficiales del expediente INMOVAL.</p>
      </header>
        ...
        <TabsList className="flex-wrap h-auto">
          {CATS.map((c) => (
            <TabsTrigger key={c.key} value={c.key} className="text-xs">
              {c.label} <span className="ml-1 text-muted-foreground">({(avaluo.fotos[c.key] ?? []).length})</span>
            </TabsTrigger>
          ))}
        </TabsList>
        {CATS.map((c) => (
          <TabsContent key={c.key} value={c.key} className="mt-4">
            <Card className="p-6 border-dashed border-2 text-center cursor-pointer hover:border-primary transition-colors"
              onClick={() => fileRefs.current[c.key]?.click()}
              onDragOver={(e) => e.preventDefault()}
              onDrop={(e) => { e.preventDefault(); handleFiles(c.key, e.dataTransfer.files); }}>
              <Upload className="h-8 w-8 mx-auto text-muted-foreground mb-2" />
              <div className="text-sm">Arrastra imágenes aquí o haz clic para subir · {c.label}</div>
              <input ref={(el) => (fileRefs.current[c.key] = el)} type="file" accept="image/*" multiple className="hidden"
